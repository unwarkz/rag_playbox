...
Для кастомного образа используется Dockerfile на базе `pgvector/pgvector:pg17`. Он генерирует локаль из переменной `POSTGRES_LOCALE` и устанавливает необходимые расширения (pgvector, pg_trgm, unaccent, uuid-ossp). ParadeDB не используется из-за лицензии AGPL.
...
RAGFlow (и встроенный Infinity) требует увеличенного значения `vm.max_map_count`. По умолчанию Linux устанавливает 65530, что недостаточно для memory-mapped файлов Infinity. Установка 262144 необходима для стабильной работы.
...
> **Образ:** Собственный Dockerfile на базе `pgvector/pgvector:pg17`. Включает pgvector, pg_trgm, unaccent, uuid-ossp. Локаль генерируется из `POSTGRES_LOCALE`.
>
...
>
> **Neo4j graph memory включён** — переменные `MEM0_GRAPH_STORE_*` подключают Neo4j для графовой памяти.
...
>
> **Все переменные вынесены в `.env`** — выбор vector store, провайдеров LLM/embedding, моделей настраивается через переменные окружения.
...
>
> **Все настройки (vector store, LLM, embedding) вынесены в переменные `.env`.**
...
**Примечание:** Локаль берётся из переменной `POSTGRES_LOCALE`. Init SQL генерируется через entrypoint в compose-файле (см. `docker-compose-postgres.yml`).
...
**Примечание:** Конфиг LiteLLM встроен в compose-файл через entrypoint heredoc (для совместимости с Portainer). См. `docker-compose-litellm.yml`.
